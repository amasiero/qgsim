...
public class Similarity {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = i; j < matrix.length; j++) {
				...
				for (int k = 0; k < data.get(0).getRegister().size(); k++) {
					sqSum += Math.pow((data.get(i).getRegister().get(k) - data
							.get(j).getRegister().get(k)), 2);
				}
	...
	public static short[][] dataNormalization(float[][] data) {
		...
		for (int i = 0; i < data.length; i++) {
			for (int j = i; j < data.length; j++) {
				newData[i][j] = (short) ((1 - ((data[i][j] - minValue) / (maxValue - minValue)))*1000);
	 ...
	 * @return
	 */
	public static DataRegister centroid(List<Short> aux,
